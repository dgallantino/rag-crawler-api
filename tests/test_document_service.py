...
def test_create_document_upload_persists_and_triggers(
    mock_trigger, db_session, test_user
) -> None:
    user, _ = test_user

    document = create_document_upload(db_session, user, "guide.md", "# Guide\n\nHello world.")
...
def test_create_document_upload_raises_on_duplicate(
    ...
    with pytest.raises(DocumentConflictError, match="guide.md"):
        create_document_upload(db_session, user, "guide.md", "# New content")
...
def test_get_document_status_returns_queued_without_redis(db_session, test_user) -> None:
    user, _ = test_user
    document = Document(
        system_user_id=user.id,
        url="file://pending.md",
...
def test_get_document_status_returns_db_status_without_redis(db_session, test_user) -> None:
    user, _ = test_user
    document = Document(
        system_user_id=user.id,
        url="file://done.md",
...
def test_get_document_status_reads_redis_when_available(
    mock_redis_status, db_session, test_user
) -> None:
    user, _ = test_user
    document = Document(
        system_user_id=user.id,
...
def test_get_document_status_includes_error_message(db_session, test_user) -> None:
    user, _ = test_user
    document = Document(
        system_user_id=user.id,
        url="file://failed.md",
